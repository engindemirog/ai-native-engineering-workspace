---
description: "Change request — triage (bug / trivial / change), then the lane the behavior demands. See workflows/change-request.md."
---
Read AGENTS.md, workflows/change-request.md, and workflows/segments.md. Handle the change request
${input:target} (include the work item key).
...
/build → /review (`reviewer` agent or fresh chat) → /verify (before/after screenshots for UI) →
ship, asking at every gate. Entry checks stay: the plan gate is never skipped.
